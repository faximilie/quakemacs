...
for only the buffer that was saved. It redefines and uses the locally scoped
[[file:~/.config/emacs/.local/straight/repos/org/lisp/org-id.el::id-regexp][id-regexp in org-id.el]]
...
PATH should be a topic that can be thrown at the man command."
  ...
  (defun org-man-export (link description format)
    ...
    (let* ((regexp "\\(\\w+\\)(\\([0-9]+\\))")
           ;; TODO helper function for better matching strings
           (topic (progn
                    (string-match regexp link)
                    (match-string 1 link)))
           (section (progn
                      (string-match regexp link)
                      (match-string 2 link)))
           (path (format "https://man7.org/linux/man-pages/%s/%s"
                         (concat topic section)
                         (format "%s.%s.html" topic section)))
...
(use-package! copilot
  ...
  :bind ((kbd "C-TAB") #'copilot-accept-completion)
